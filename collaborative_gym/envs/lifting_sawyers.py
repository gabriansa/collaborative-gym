...
class LiftTaskSawyersEnv(BaseEnv, MultiAgentEnv):
    def __init__(self):
        self.my_robots = {}
        self.obs_len_robots = {}
        ...
        super(LiftTaskSawyersEnv, self).__init__()
    ...
    def reset(self):
        super(LiftTaskSawyersEnv, self).reset()
        self.create_world()

        # Position robot 1
        self.my_robots['robot_1'].set_base_pos_orient([0.22,-1.2,1], [0,0,np.pi/2])

        # Position robot 2
        self.my_robots['robot_2'].set_base_pos_orient([-0.22,1.2,1], [0,0,-np.pi/2])

        # Create and posiiton a table
        self.table = Object()
        self.table.init('table', self.directory, self.id, self.np_random)
        self.table.set_base_pos_orient([0,0,0], [0,0,np.pi/2])
