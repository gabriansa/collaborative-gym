...
class BalanceTaskSawyersEnv(BaseEnv, MultiAgentEnv):
    def __init__(self):
        self.my_robots = {}
        self.obs_len_robots = {}
        ...
        super(BalanceTaskSawyersEnv, self).__init__()
    ...
    def step(self, action):
        ...
        # Apply noise
        F_noise = random.uniform(-10, 10)
        x_pos_noise = random.uniform(-0.2, 0.2)
        y_pos_noise = random.uniform(-0.45, 0.45)
        p.applyExternalForce(self.balancing_board.body, -1, [0,0,F_noise], [x_pos_noise,y_pos_noise,0], p.WORLD_FRAME)
    ...
    def reset(self):
        super(BalanceTaskSawyersEnv, self).reset()
        self.create_world()

        # Position robot 1
        self.my_robots['robot_1'].set_base_pos_orient([0,-1.8,1], [0,0,np.pi/2])
    ...
    def create_balancing_board(self):
        ...
        self.balancing_board.set_base_pos_orient([0,0,0.7], [0,0,0])
